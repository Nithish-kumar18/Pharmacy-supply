...
public class MedRepScheduleController {
	final Logger log = LoggerFactory.getLogger(MedRepScheduleServiceApplication.class);
	@Autowired
	...
	@GetMapping("/getmedicine/{treatingAilment}")
	public ResponseEntity<String[]> getMedicinesByTreatingAilment(@RequestHeader(name = "Authorization") String token, @PathVariable("treatingAilment") String treatingAilment) {
		log.info("Start");
		
		final ResponseEntity<String[]> responseEntity = ResponseEntity.of(Optional.of(medicineStockClient.getMedicinesByTreatingAilment(token,treatingAilment)));
